...
\begin{figure}[H]
  ...
  \caption{Dispersion relation diagnostic extracted from probe signals of mode-sweep simulations (phase-velocity error).}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{pic/dissipation_amplification_central.png}
  \caption{Amplification-factor diagnostic for the central difference method, estimated from simulation data.}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{pic/dissipation_amplification_newmark.png}
  \caption{Amplification-factor diagnostic for Newmark $(\beta,\gamma)=(1/4,1/2)$, estimated from simulation data.}
\end{figure}

In this revised workflow, Figures~4.4--4.6 are not generated from direct
closed-form plotting. Instead, we run mode-sweep simulations
$(m_x,m_y)=(1,1),\dots,(14,1)$ and read probe signals
$u(t,\mathbf x_{p1}),u(t,\mathbf x_{p2})$ from the result folders. For each run,
we compute
\[
k=\pi\sqrt{m_x^2+m_y^2},\qquad kh=k\,h_{\min},
\]
estimate $\omega_h^d$ from the probe time series (zero-crossing/FFT), and form
$\omega_h^d/\omega_{\text{exact}}$ with $\omega_{\text{exact}}=k$. The
amplification factor is estimated from early/late envelope amplitudes as an
effective per-step gain $|G|$.
