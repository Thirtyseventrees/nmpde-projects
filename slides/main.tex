...
\begin{frame}{Experiment Matrix and Metrics}
  \begin{itemize}
    \item Exact benchmark solution for error evaluation:
      \[
        u(x,y,t)=\sin(\pi x)\sin(\pi y)\cos(\omega t),\quad
        \omega=\sqrt{2}\pi.
      \]
...
\begin{frame}{Dispersion Diagnostic}
  ...
  \vspace{0.2em}
  \begin{itemize}
    \item Axis meaning: horizontal axis is $kh$ (non-dimensional wavenumber),
          vertical axis is $\omega_h^d/\omega_{\text{exact}}$ (phase-speed ratio).
...
\begin{frame}{Dissipation Diagnostic (Amplification Factor)}
  ...
  \begin{itemize}
    \item Axis meaning: horizontal axis is $kh$ (non-dimensional wavenumber),
          vertical axis is $|G|$ (amplitude amplification per time step).
